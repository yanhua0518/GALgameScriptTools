...
if len(sys.argv) < 3:
    print ("Usage: "+sys.argv[0]+" <Scene.pck> <Scene\> [Scene.pck2]")
    quit()
...
if len(sys.argv) < 4:
    ...
    outFN=sys.argv[3]
inF=sys.argv[0][:sys.argv[0].rfind("\\")+1]+sys.argv[2]+"\\"
try:
...
for n in range(0,header.SceneDataCount):
    ...
    ssSize=os.path.getsize(inF+fileName)
    data=ssFile.read()
    #No compression yet, this is fake.
    dataSize=ssSize+int(ssSize/8)+8
    if not ssSize%8==0:
        dataSize+=1
    compData=struct.pack('2I',dataSize,ssSize)
